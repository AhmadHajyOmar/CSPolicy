...
for (var osystem of op) {
    ...
    if(csp_policies.length > 0) {
        ...
        for (let i = 0; i < csp_safe.length; i++) {
            ...
            let fileName = `csp_${arr_comp[0][1]}_${arr_comp[4][1]}_${arr_comp[5][1]}.json`
        ...
        if(viewport_arr.length > 0) {
            ...
            for (let i = 0; i < viewport_arr.length; i++) {
                ...
                let fileName_viewport = `${dev_mod}.json` 
                    let json_viewport = {};
        ...
        if(arraysEquals(viewport_res, viewport_arr)) {
            ...
            fs.writeFileSync(`./viewport_comp/compResultViewport_${osystem}.json`, JSON.stringify(json_viewport_result))
        ...
        fs.writeFileSync(`./compare/compResult_${osystem}.json`, JSON.stringify(json_comp))
    }
